...
public class CircularLinkedList {
    ...
    void insertLeft(int data){
        ...
        if(root == null){  // if there is no node then this condition executes
            ...
            n.next = root;   // connect next of newly created node to root node which is already created by user
    ...
    void deleteLeft(){
        if(root == null){   //
            ...
            if(root.next == root){
                ...
                root= root.next; 
                last.next = root; //move root to next node and t will automatically get deleted due to automatic garbage collection  
            }
    ...
    void insertRight(int data){  
       Node n = new Node(data);  // create new node with reference n 
        if(root == null){   // check wehter root is null or not if null then assign root to newly created node
            root =last= n; 
            last.next = root; 
        }
        else{
        ...
        if(root == null){   
         ...
         if(root.next==root){
            ...
            while(t.next != root){  //or t!= last
                t2=t;        //pass t2 behind the t . t tell t2 wether next node is empty or not if found then t2 will null the node at which he is present  
               t= t.next;  
             }  
             last =t2;
